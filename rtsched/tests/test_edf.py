...
def test_1():
    ...
    tsks = [Task(wcet=w, period=p, deadline=d) for w, p, d
            in zip(wcets, periods, deadlines)]
...
def dbf(tsks: List[Task], t: int) -> int:
    ...
    return sum([max(0,
                    math.floor(Fraction(
                        t + tsk.period + tsk.jitter - tsk.deadline,
...
def qpa(tsks: List[Task]):
    ...
    def max_dj(t):
        ...
        for tsk in tsks:
            if tsk.dj < t:
                d = math.floor(Fraction(t - tsk.dj, tsk.period)) * tsk.period + tsk.dj
                if d == t:
                    d -= tsk.period
                if d > max:
                    max = d
...
@pytest.mark.parametrize("num_systems, n, max_wcet, sum_util, deadline_type, sum_dens,"
                         "max_jitter",[
                             (100, 2, 100, 0.75, 'constrained', 0.9, 10),
                             (1000, 5, 100, 0.80, 'arbitrary', 0.9, 10)
                         ])
