...
"""Abhishek: We have modified the file to pass an rng as an argument to the drs
and calls to random.* are replaced by rng.*. This change is made so that we
...
if USE_MPMATH:
    ...
    def scaled_dirichlet(rng: np.random.Generator, n: int, u: float) -> List[float]:
        """A Python implementation of the Standard Dirichlet. This is
        faster than using scipy.dirichlet for single values"""
        assert n > 0
        if n == 1: return [u]
        intermediate = [-math.log(1 - rng.random()) for _ in range(n)]
        divisor = sum(intermediate)
...
def drs_i(rng: np.random.Generator, n: int, sumu: float,
    ...
    if lower_bounds is not None:
        ...
        transformed_problem_result = drs_i(rng, n, sumu - sum(lower_bounds),
    ...
    for count in range(DRS_RETRIES):
        initial_point = scaled_dirichlet(rng, n, 1)
...
def drs(rng: np.random.Generator, n: int, sumu: float,
    ...
    return drs_i(rng, n, sumu, upper_bounds, lower_bounds).output_point
