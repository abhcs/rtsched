...
@pytest.mark.parametrize("wcets, periods, expected",
                         [([20, 10, 33], [40, 50, 150], 143), ([1], [4], 1),
                          ([1, 1], [4, 5], 2), ([1, 1, 3], [4, 5, 9], 7),
                          ([1, 1, 3, 3], [4, 5, 9, 18], 18), ([1], [2], 1),
                          ([1, 5], [2, 20], 10), ([1, 5, 1], [2, 20, 20], 12)])
...
def rta(tsks: List[Task]):
    ...
    def rbf(t: int):
        return sum(
            math.ceil(Fraction(t + tsk.jitter, tsk.period)) * tsk.wcet
            for tsk in tsks)
...
def test_2():
    ...
    tsks = [
        Task(wcet=w, period=p, deadline=d, jitter=j)
...
@pytest.mark.parametrize(
    "tsks, expected",
    [([Task(wcet=i, period=j)
       for i, j in zip([20, 10, 33], [40, 50, 150])], True),
     ([Task(wcet=i, period=j)
       for i, j in zip([1, 1, 3, 3], [4, 5, 9, 18])], True),
...
@pytest.mark.parametrize(
    ...
    "max_jitter", [(1000, 2, 1, 100, 0.75, 'constrained', 0.9, 10),
                   (1000, 4, 1, 100, 0.9, 'implicit', 0.9, 10),
                   (20, 16, 1, 100, 0.9, 'implicit', 0.9, 10)])
